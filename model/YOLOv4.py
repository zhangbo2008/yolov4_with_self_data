...
class SpatialPyramidPooling(nn.Module):
    ...
    def forward(self, x):
        x = self.head_conv(x)
        features = [maxpool(x) for maxpool in self.maxpools] # 这个ssp表示多尺度的做pool,所以这里面一共有3种分辨率的结果.
        features = torch.cat([x] + features, dim=1) # 特征全部拼起来
